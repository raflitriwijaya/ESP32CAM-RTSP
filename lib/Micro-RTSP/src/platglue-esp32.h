...
{
    // endPacket() fails with errno 12 (ENOMEM) when the WiFi/LWIP TX buffers are
    // momentarily full — common while a frame's RTP fragments go out in a burst.
    // Dropping the fragment there causes visible stutter/lag in the RTSP client,
    // so retry a few times with a short yield to let the TX path drain. delay(1)
    // yields to the WiFi driver task (~1 ms) between attempts. Bounded retries
    // (CLAUDE.md §5): after UDP_SEND_MAX_ATTEMPTS we drop this one fragment rather
    // than stall the streaming task.
    const int UDP_SEND_MAX_ATTEMPTS = 3;
    for (int attempt = 0; attempt < UDP_SEND_MAX_ATTEMPTS; attempt++) {
        sockfd->beginPacket(destaddr, destport);
        sockfd->write((const uint8_t *) buf, len);
        if (sockfd->endPacket())
            return len;          // sent OK
        delay(1);                // let TX buffers drain, then retry
    }

    printf("error sending udp packet (dropped after %d attempts)\n", UDP_SEND_MAX_ATTEMPTS);
    return len;
}
